...
open InMemoryStore
let foo(zm:ZoneMemory, minx, miny, maxx, maxy, eachWidth, eachHeight, gameMapAspect, margin, onHover:Event<_>, onLeave:Event<_>) =
    let MAX = 100
    ...
    c, mapBmps, nonMainBmps, linkages
    ...
    for zi = 0 to zma.GetLength(0)-1 do
        for zj = 0 to zma.GetLength(1)-1 do
            ...
            if maxx >= minx then // there was at least one screenshot
                ...
                let imgCanvas, mapBmps, nonMainBmps, linkages = foo(zm, minx, miny, maxx, maxy, eachWidth, eachHeight, gameMapAspect, MARGIN, onHover, onLeave)
...
(*
let MakeDualFeatureMap(owner, zm1:ZoneMemory, zm2:ZoneMemory, minx, maxx, miny, maxy) =
    // framing layout
    let gameMapAspect = 
        let _mx,_my,mw,mh = GameSpecific.MapArea
        float mw / float mh
    ...
    let highlightRect1, highlightRect2 = makeHighlightRect(), makeHighlightRect()
    // zones
    let eachWidth  = TOTALW / float(zma.GetLength(0)) |> int
    let eachHeight = TOTALH / float(zma.GetLength(1)) |> int
    for zi = 0 to zma.GetLength(0)-1 do
        for zj = 0 to zma.GetLength(1)-1 do
            ...
            if maxx >= minx then // there was at least one screenshot
                ...
                let label = new TextBox(IsReadOnly=true, FontSize=12., Text=BackingStoreData.theGame.ZoneNames.[zm.Zone], BorderThickness=Thickness(1.), 
                                        Foreground=Brushes.Black, Background=Brushes.White, TextAlignment=TextAlignment.Center,
                                        Width=float eachWidth-4.*MARGIN, Height=float(LABELH))
                Utils.canvasAdd(c, label, ulx+2.*MARGIN, uly+float eachHeight-float LABELH)
                let WW = PICH / float GameSpecific.MapAreaRectangle.Height * float GameSpecific.MapAreaRectangle.Width
                img.MouseMove.Add(fun ea ->
                    let pos = ea.GetPosition(img)
                    let di, dj = int(pos.X / float w), int(pos.Y / float h)
